...
export function AuthProvider({ children }) {
  ...
  const handleLogin = async (email, password) => {
    setLoading(true);
    try {
      const data = await api.login({ email: email.toLowerCase().trim(), password: password.trim() });
      if (data?.user) {
        setUser(data.user);
      ...
      return { success: false, error: "Unable to connect to server. Try again." };
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AuthContext.Provider value={{ user, isAuthenticated, handleLogin, logout, loading, initSession }}>
