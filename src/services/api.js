...
export async function fetchWithSession(endpoint, options = {}) {
  ...
  const config = {
    ...
    headers,
    credentials: "include",
    body: method !== "GET" ? (isFormData ? options.body : JSON.stringify(options.body || {})) : undefined,
  };
...
const api = {
  ...
  getFreelancerTaskById: (task_id) => fetchWithSession(`/freelancer/tasks/${task_id}`),  // <-- NEW
  updateFreelancerTask: (task_id, data) =>
    fetchWithSession(`/freelancer/tasks/${task_id}`, { method: "PATCH", body: data }),   // <-- NEW
